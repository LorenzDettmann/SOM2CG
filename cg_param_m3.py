...
def param_bead(bead,bead_smi,ring_size,frag_size,ring,qbead,don,acc,DG_data,matched_maps,path_matrix):
...
def write_gro(mol_name,bead_types,coords0,mol,gro_name):
    #write gro file
...
def write_itp(mol_name,bead_types,coords0,charges,all_smi,A_cg,ring_beads,beads,mol,nconfs,virtual,real,masses,resname_list,itp_name):
...
def write_atoms(itp,A_cg,mol_name,bead_types,charges,all_smi,coords,ring_beads,virtual,masses,resname_list):
    ...
    for b in range(len(bead_types)):
        itp.write('{:5d}{:>6}{:5d}{:>5}{:>5}{:5d}{:>10.3f}{:>10.3f};{}\n'.format(b+1,bead_types[b],1,resname_list[b],'CG'+str(b+1),b+1,charges[b],masses[b],all_smi[b]))
...
def write_virtual_sites(itp,virtual_sites,beads):
    ...
    for vs in vs_iter:
        ...
        if len(cs) == 4:
            ...
            itp.write('{:5d}{:3d}{:3d}{:3d}{:5d}{:7.3f}{:7.3f}\n'.format(vs+1,cs[0][0]+1,cs[1][0]+1,cs[2][0]+1,1,cs[0][1],cs[1][1]))
        elif len(cs) == 2:
            #itp.write('{:5d}{:3d}{:5d}{:7.3f}{:5d}{:7.3f}\n'.format(vs+1,3,cs[0][0]+1,cs[0][1],cs[1][0]+1,cs[1][1]))
